...
function Login() {
  ...
  function handleOnChange(type, e) {
    ...
    if (type === "email") {
      ...
      hasOnlyDigits(inputValue)
        ? setValueCategory("mobile")
        : setValueCategory("email");
    } else {
      setValue({
        email: value.email,
        password: inputValue,
  ...
  async function handleOnSignIn() {
    if (value.email === "" || value.password === "") {
      ...
      if (
        ((valueCategory === "mobile" && value?.email?.length !== 10) ||
          (valueCategory === "email" && !validEmail)) &&
        ...
        if (res.status === "SUCCESS") {
          if (res.isUserAlreadyExist) {
            if (
              res.alreadyExistedUser.password === value.password &&
              res.alreadyExistedUser.emailOrMobile === value.email
            ) {
              const updatedUser = {
                ...res.userObject,
                ...res.alreadyExistedUser,
              };
              setIsUserLoggedIn(true)
              dispatch(setCurrentUser({ ...user, ...updatedUser }));
              setLocalUser({ ...user, ...updatedUser })
              router.push("/");
        ...
        <Box className={`${classes.flexBox} overlay`}>
          <Box className={classes.mainContainer}>
            ...
            {value.showNewAccountModal && (
              <Box p={2} className={classes.signUpErr}>
                {globalLanguage === "en" ? (
                  ...
                  <Typography>
                    माफ़ करें, हमें इस ईमेल ऐड्रेस से जुड़ा अकाउंट नहीं मिला.
                    कृपया फिर कोशिश करें या{" "}
                    <Link style={{ color: "black" }} href="/">
                      नया अकाउंट बनाएं.
                    </Link>
                  </Typography>
            ...
            <CommonTextField
              ...
              type="text"
              maxLength={
                valueCategory && valueCategory === "mobile" ? 10 : 3000
              }
              handleOnChange={(e) => handleOnChange("email", e)}
              label={languageText.EMAIL_OR_PHONE_NUMBER}
            />

            {value.showEmailErrorMessage && (
              <Typography className={classes.errorMessage}>
                {languageText?.EMAIL_VALIDATION_TEXT}
              </Typography>
            )}
            ...
            <Typography variant="caption" className={classes.captchaText}>
              {languageText.THIS_PAGE_IS_PROTECTED}{" "}
              {!hideLeanMore && (
                <span
            ...
            {hideLeanMore && (
              <Typography variant="caption" className={classes.captchaText}>
                ...
                <Link
                  ...
                  className={classes.learnMoreText}
                >
                  {languageText.TERMS_OF_SERVICE}
                </Link>
                {languageText.IS_USED_FOR_PROVIDING}
              </Typography>
            )}
          </Box>
        </Box>
